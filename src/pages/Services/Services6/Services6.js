...
const Services6 = () => {
    ...
    useEffect(() => {
      if (clicked || !componentLoaded) {
        ...
        const data = []
        db.collection('services').where("parentTitle", "==", "خدمات الصرف الصحي والنفايات").get()
        .then(snapshot => {
      ...
      <section className="trans">
        <div className='HeaderAds'>
          ...
          <Modal visible={showModal} closeModal={() => setShowModal(false)} >
            <ServicesForm parentTitle="خدمات الصرف الصحي والنفايات" closeModal={() => {setShowModal(false);handleClick()}} />
          </Modal>
      </section>
    )
  }
