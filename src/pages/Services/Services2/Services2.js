...
const Services2 = () => {
    ...
    useEffect(() => {
      if (clicked || !componentLoaded) {
        ...
        const data = []
        db.collection('services').where("parentTitle", "==", "خدمات الصحة والبيئة").get()
        .then(snapshot => {
      ...
      <section className="trans">
        <div className='HeaderAds'>
          ...
          <Modal visible={showModal} closeModal={() => setShowModal(false)} >
            <ServicesForm parentTitle="خدمات الصحة والبيئة" closeModal={() => {setShowModal(false);handleClick()}} />
          </Modal>
      </section>
    )
  }
