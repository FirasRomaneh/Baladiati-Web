...
const Services5 = () => {
    ...
    useEffect(() => {
      if (clicked || !componentLoaded) {
        ...
        const data = []
        db.collection('services').where("parentTitle", "==", "خدمات الشوارع والطرق").get()
        .then(snapshot => {
      ...
      <section className="trans">
        <div className='HeaderAds'>
          ...
          <Modal  visible={showModal} closeModal={() => setShowModal(false)} >
            <ServicesForm parentTitle="خدمات الشوارع والطرق" closeModal={() => {setShowModal(false);handleClick()}} />
          </Modal>
      </section>
    )
  }
