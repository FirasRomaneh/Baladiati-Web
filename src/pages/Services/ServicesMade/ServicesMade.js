...
import SingleServicesMade from './SingleServicesMade';
...
const ServicesMade = () => {
    ...
    useEffect(() => {
      if (clicked || !componentLoaded) {
        ...
        const EmployeesRef = db.collection('serviceMade');
        EmployeesRef.get()
        .then(snapshot => {
          snapshot.forEach(doc => {
            ...
            if(newData.amount){
                Total = Total + parseInt(newData.amount, 10)
            }
            newData.id = doc.id;
            newData.submitDate = newDate;
            newData.date = date;
            if (!Titles.includes(newData.title)) {
                Titles.push(newData.title)
            }
            data.push(newData);
          });
          setTitles(Titles)
    ...
    }, [Type, ViewCalender, clicked, componentLoaded, search, selectedDate, title, titles]);
      ...
      <section className="trans">
        <div className='HeaderEmp'>
          ...
          <select onChange={handleFilter2} className="selectOption2">
              ...
              <option value="Yes">الخدمات المنجزة</option>
              <option value="No">الخدمات الغير منجزة</option>
          </select>
          <input
            type="number"
        ...
        <div className='HeaderSer'>
            <Money className='icon'/> Paid Amounts: {PaidAmount}
        </div>
        </div>
        {ViewCalender && ( <div className='calendar-container'>
          ...
          <div className="trans_content">
              ...
              <SingleServicesMade Data={item} key={item.id} handleClick={handleClick}/>
              ))}
          </div>
      </section>
    )
